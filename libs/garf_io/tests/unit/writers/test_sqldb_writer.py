...
from gaarf_io.writers import sqldb_writer
...
class TestSqlAlchemyWriter:
  ...
  def test_write_single_column_report_returns_correct_data(
    ...
    assert gaarf_report.GaarfReport.from_pandas(df) == single_column_data
  ...
  def test_write_multi_column_report_with_arrays_returns_concatenated_strings(
    ...
    expected_report = gaarf_report.GaarfReport(results, columns)
    ...
    assert gaarf_report.GaarfReport.from_pandas(df) == expected_report
